...
try {
	...
	step("brand-artisan pdf");
	run("npx brand-artisan pdf demo");
	const pdf = readFileSync(join(APP, "out", "demo.pdf"));
	assert.ok(pdf.subarray(0, 5).equals(Buffer.from("%PDF-")), "en-tete PDF");
